...
class TritosGUI(Gtk.Window):
    ...
    def _navigate_to(self, section):
        ...
        dispatch = {
            ...
            "file-compress": self._show_file_compress,
        }
    ...
    def _show_files(self):
        ...
        for label, handler in [
            ...
            ("📦 Comprimir", lambda w: self._files_compress_dialog()),
            ("📦 Descomprimir", lambda w: self._files_decompress_dialog())]:
            btn = Gtk.Button(label=label)
    ...
    def _files_refresh(self):
        ...
        for name in entries:
            ...
            cmp_btn = Gtk.Button(label="📦")
            cmp_btn.get_style_context().add_class("del-btn")
            cmp_btn.get_style_context().remove_class("del-btn")
            cmp_btn.get_style_context().add_class("back-btn")
            cmp_btn.connect("clicked", lambda w, p=full: self._files_compress_one(p))
            row.pack_end(cmp_btn, False, False, 0)
    ...
    def _files_compress_one(self, path):
        COMPRESS_BIN = "/usr/bin/tritos_compress"
        if not os.path.exists(COMPRESS_BIN):
            COMPRESS_BIN = os.path.join(SCRIPT_DIR, "tritos_compress")
        out = path + ".trc5"
        cmd = [COMPRESS_BIN, "-c", "-m", "ternary", path, out]
        try:
            r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            dlg = Gtk.Dialog(
                title="Compresión", parent=self,
                flags=Gtk.DialogFlags.MODAL,
                buttons=("OK", Gtk.ResponseType.OK))
            tv = Gtk.TextView()
            tv.set_editable(False)
            tv.set_monospace(True)
            tv.get_buffer().set_text(r.stdout + r.stderr)
            tv.get_style_context().add_class("output-text")
            dlg.get_content_area().pack_start(tv, True, True, 0)
            dlg.set_default_size(450, 200)
            dlg.show_all()
            dlg.run()
            dlg.destroy()
            self._files_refresh()
    ...
    def _files_decompress_dialog(self):
        dialog = Gtk.Dialog(
            title="Descomprimir .trc5", parent=self,
            flags=Gtk.DialogFlags.MODAL,
            buttons=("Descomprimir", Gtk.ResponseType.OK,
                     "Cancelar", Gtk.ResponseType.CANCEL))
        box = dialog.get_content_area()
        box.set_spacing(6)
        box.set_margin_start(10)
        box.set_margin_end(10)
        box.set_margin_top(10)
        box.add(Gtk.Label(label="Archivo .trc5:"))
        file_entry = Gtk.Entry()
        file_entry.set_text("")
        file_entry.set_placeholder_text("Ruta del archivo .trc5")
        box.add(file_entry)
        browse_btn = Gtk.Button(label="Examinar")
        def on_browse(w):
            dlg = Gtk.FileChooserDialog(
                title="Seleccionar .trc5", parent=dialog,
                action=Gtk.FileChooserAction.OPEN)
            dlg.add_buttons("Cancelar", Gtk.ResponseType.CANCEL,
                            "Abrir", Gtk.ResponseType.OK)
            filt = Gtk.FileFilter()
            filt.set_name("Archivos .trc5")
            filt.add_pattern("*.trc5")
            dlg.add_filter(filt)
            resp = dlg.run()
            if resp == Gtk.ResponseType.OK:
                file_entry.set_text(dlg.get_filename())
            dlg.destroy()
        ...
        resp = dialog.run()
        if resp == Gtk.ResponseType.OK:
            inp = file_entry.get_text().strip()
            if inp and os.path.exists(inp):
                COMPRESS_BIN = "/usr/bin/tritos_compress"
                if not os.path.exists(COMPRESS_BIN):
                    COMPRESS_BIN = os.path.join(SCRIPT_DIR, "tritos_compress")
                out = inp[:-5] if inp.endswith(".trc5") else inp + ".out"
                cmd = [COMPRESS_BIN, "-d", inp, out]
                try:
                    r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
                    dlg2 = Gtk.Dialog(
                        title="Descompresión", parent=self,
                        flags=Gtk.DialogFlags.MODAL,
                        buttons=("OK", Gtk.ResponseType.OK))
                    tv = Gtk.TextView()
                    tv.set_editable(False)
                    ...
                    tv.get_buffer().set_text(r.stdout + r.stderr)
                    tv.get_style_context().add_class("output-text")
                    dlg2.get_content_area().pack_start(tv, True, True, 0)
                    dlg2.set_default_size(450, 200)
                    dlg2.show_all()
                    dlg2.run()
                    dlg2.destroy()
                    self._files_refresh()
    ...
    def _files_compress_dialog(self):
        dialog = Gtk.Dialog(
            title="Comprimir archivo", parent=self,
            flags=Gtk.DialogFlags.MODAL,
            buttons=("Comprimir", Gtk.ResponseType.OK,
                     "Cancelar", Gtk.ResponseType.CANCEL))
        box = dialog.get_content_area()
        box.set_spacing(6)
        box.set_margin_start(10)
        box.set_margin_end(10)
        box.set_margin_top(10)
        box.add(Gtk.Label(label="Archivo a comprimir:"))
        file_entry = Gtk.Entry()
        file_entry.set_placeholder_text("Ruta del archivo")
        box.add(file_entry)
        browse_btn = Gtk.Button(label="Examinar")
        def on_browse(w):
            dlg = Gtk.FileChooserDialog(
                title="Seleccionar archivo", parent=dialog,
                action=Gtk.FileChooserAction.OPEN)
            dlg.add_buttons("Cancelar", Gtk.ResponseType.CANCEL,
                            "Abrir", Gtk.ResponseType.OK)
            resp = dlg.run()
        ...
        browse_btn.connect("clicked", on_browse)
        box.add(browse_btn)
        box.add(Gtk.Label(label="Modo:"))
        mode_combo = Gtk.ComboBoxText()
        mode_combo.append_text("ternary")
        mode_combo.append_text("crt")
        ...
        if resp == Gtk.ResponseType.OK:
            ...
            if inp and os.path.exists(inp):
                ...
                if not os.path.exists(COMPRESS_BIN):
                    COMPRESS_BIN = os.path.join(SCRIPT_DIR, "tritos_compress")
                out = inp + ".trc5"
                cmd = [COMPRESS_BIN, "-c", "-m", mode, inp, out]
                try:
                    r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
                    dlg2 = Gtk.Dialog(
                        title="Compresión", parent=self,
    ...
    def _show_file_compress(self):
        content = self._make_page("📦 COMPRESOR TERNARIO")

        COMPRESS_BIN = "/usr/bin/tritos_compress"
        if not os.path.exists(COMPRESS_BIN):
            COMPRESS_BIN = os.path.join(SCRIPT_DIR, "tritos_compress")

        f1 = Gtk.Frame(label=" Archivo de Entrada ")
        f1.get_style_context().add_class("card")
        h1 = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        h1.set_margin_start(8)
        h1.set_margin_end(8)
        h1.set_margin_top(8)
        h1.set_margin_bottom(8)
        f1.add(h1)
        self._fc_input = Gtk.Entry()
        self._fc_input.set_placeholder_text("Ruta del archivo a comprimir/descomprimir...")
        self._fc_input.set_hexpand(True)
        h1.pack_start(self._fc_input, True, True, 0)
        browse_in = Gtk.Button(label="Examinar")
        browse_in.get_style_context().add_class("back-btn")
        browse_in.connect("clicked", lambda w: self._fc_browse_file(self._fc_input, False))
        h1.pack_start(browse_in, False, False, 0)
        content.pack_start(f1, False, False, 0)

        f2 = Gtk.Frame(label=" Archivo de Salida ")
        f2.get_style_context().add_class("card")
        h2 = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        h2.set_margin_start(8)
        h2.set_margin_end(8)
        h2.set_margin_top(8)
        h2.set_margin_bottom(8)
        f2.add(h2)
        self._fc_output = Gtk.Entry()
        self._fc_output.set_placeholder_text("Ruta del archivo de salida (opcional)...")
        self._fc_output.set_hexpand(True)
        h2.pack_start(self._fc_output, True, True, 0)
        browse_out = Gtk.Button(label="Examinar")
        browse_out.get_style_context().add_class("back-btn")
        browse_out.connect("clicked", lambda w: self._fc_browse_file(self._fc_output, True))
        h2.pack_start(browse_out, False, False, 0)
        content.pack_start(f2, False, False, 0)

        opt_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)
        opt_box.set_margin_top(4)
        mode_lbl = Gtk.Label(label="Modo:")
        mode_lbl.get_style_context().add_class("param-label")
        opt_box.pack_start(mode_lbl, False, False, 0)
        self._fc_mode = Gtk.ComboBoxText()
        self._fc_mode.append_text("ternary")
        self._fc_mode.append_text("crt")
        self._fc_mode.set_active(0)
        opt_box.pack_start(self._fc_mode, False, False, 0)
        content.pack_start(opt_box, False, False, 0)

        btn_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        btn_box.set_margin_top(4)
        compress_btn = Gtk.Button(label="Comprimir (-c)")
        compress_btn.get_style_context().add_class("run-btn")
        compress_btn.connect("clicked", lambda w: self._fc_run_compress(COMPRESS_BIN))
        btn_box.pack_start(compress_btn, False, False, 0)
        decompress_btn = Gtk.Button(label="Descomprimir (-d)")
        decompress_btn.get_style_context().add_class("gen-btn")
        decompress_btn.connect("clicked", lambda w: self._fc_run_decompress(COMPRESS_BIN))
        btn_box.pack_start(decompress_btn, False, False, 0)
        info_btn = Gtk.Button(label="Info (-i)")
        info_btn.get_style_context().add_class("back-btn")
        info_btn.connect("clicked", lambda w: self._fc_run_info(COMPRESS_BIN))
        btn_box.pack_start(info_btn, False, False, 0)
        content.pack_start(btn_box, False, False, 0)

        of = Gtk.Frame(label=" Resultado ")
        of.get_style_context().add_class("card")
        self._fc_output_view = Gtk.TextView()
        self._fc_output_view.get_style_context().add_class("output-text")
        self._fc_output_view.set_editable(False)
        self._fc_output_view.set_monospace(True)
        self._fc_output_view.set_wrap_mode(Gtk.WrapMode.WORD_CHAR)
        sw = Gtk.ScrolledWindow()
        sw.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        sw.add(self._fc_output_view)
        of.add(sw)
        content.pack_start(of, True, True, 0)

    def _fc_browse_file(self, entry, save_mode):
        if save_mode:
            dlg = Gtk.FileChooserDialog(
                title="Guardar como", action=Gtk.FileChooserAction.SAVE)
            dlg.add_buttons("Cancelar", Gtk.ResponseType.CANCEL,
                            "Guardar", Gtk.ResponseType.OK)
        else:
            dlg = Gtk.FileChooserDialog(
                title="Seleccionar archivo", action=Gtk.FileChooserAction.OPEN)
            dlg.add_buttons("Cancelar", Gtk.ResponseType.CANCEL,
                            "Abrir", Gtk.ResponseType.OK)
        resp = dlg.run()
        if resp == Gtk.ResponseType.OK:
    ...
    def _fc_run_compress(self, bin_path):
        inp = self._fc_input.get_text().strip()
        out = self._fc_output.get_text().strip()
        mode = self._fc_mode.get_active_text()
        if not inp:
            self._fc_output_view.get_buffer().set_text("Seleccioná un archivo de entrada.")
            return
        if not out:
            out = inp + ".trc5"
            self._fc_output.set_text(out)
        cmd = [bin_path, "-c", "-m", mode, inp, out]
        self._fc_output_view.get_buffer().set_text(f"$ {' '.join(cmd)}\n\n")
        try:
            r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            self._fc_output_view.get_buffer().set_text(r.stdout + r.stderr)
        except Exception as e:
            self._fc_output_view.get_buffer().set_text(f"Error: {e}")

    def _fc_run_decompress(self, bin_path):
        inp = self._fc_input.get_text().strip()
        out = self._fc_output.get_text().strip()
        if not inp:
            self._fc_output_view.get_buffer().set_text("Seleccioná un archivo .trc5 de entrada.")
            return
        if not out:
            if inp.endswith(".trc5"):
                out = inp[:-5]
            else:
                out = inp + ".out"
            self._fc_output.set_text(out)
        cmd = [bin_path, "-d", inp, out]
        self._fc_output_view.get_buffer().set_text(f"$ {' '.join(cmd)}\n\n")
        try:
            r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            self._fc_output_view.get_buffer().set_text(r.stdout + r.stderr)
    ...
    def _fc_run_info(self, bin_path):
        inp = self._fc_input.get_text().strip()
        if not inp:
            self._fc_output_view.get_buffer().set_text("Seleccioná un archivo .trc5.")
            return
        cmd = [bin_path, "-i", inp]
        self._fc_output_view.get_buffer().set_text(f"$ {' '.join(cmd)}\n\n")
        try:
